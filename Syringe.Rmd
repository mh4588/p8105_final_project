...
![syringe litter](nycparks_syringe/images/syringe-litter.jpg){width=80%}
...
<div style="text-align: center;">
![syringe kiosk in a park (type: mailbox)](nycparks_syringe/images/syringe-box.jpg){width=80%}
</div>
...
```{r}
syringe_map = read_csv("nycparks_syringe/Parks_Syringe_Disposal_Kiosks_20241202.csv")
...
monthly_data$predicted <- predict(time_model, monthly_data)

```
```{r}
# 2. Location-based Model with unique locations
...
location_model_data$predicted <- predict(location_model, location_model_data)
```


```{r}
p11 <- ggplot(monthly_data) +
...
```{r}
p12 <- ggplot(location_model_data) +
...
```{r}
seasonal_effects <- broom::tidy(time_model) %>%
...
ggplot(seasonal_effects, 
  ...
  labs(title = "Seasonal Effects in Time Series Model",
       ...
       y = "Effect Size",
       fill = "Statistically\nSignificant") +
  scale_fill_manual(values = c("grey", "#FF6B6B"))
